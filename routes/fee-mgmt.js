...
const { ensureAuthenticated, isAdmin, updateAccessControl } = require('../helpers/auth');
...
router.get('/', [ensureAuthenticated, isAdmin], async (req, res) => {
    ...
    let feesArr = [];
    for (let index = 0; index < pendingFees.length; index++) {
        feesArr.push(pendingFees[index]._doc);
    }
    // console.log('pendingFees arr', arr);
