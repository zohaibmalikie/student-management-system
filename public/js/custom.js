...
(function () {
    'use strict';
    // let today = Date.UTC().toString();
    // document.getElementsByClassName('attendance-date').value = today;
    // alert(today)
    window.addEventListener('load', function () {
        // let today = Date.now();
        // document.getElementsByClassName('attendance-date') = today;
...
// function handlePlusBtn(bool) {
//     if (bool) {
//         document.getElementById('inPlus').style.display = 'none';
//     }

// }
function addAttendance(type) {
    // console.log(attendanceBool)
    // console.log('asdf')
    let options = {
        method: 'POST',
        headers: {
    ...
    fetch(`https://student-mgt-sys.herokuapp.com/attendance/${type === 1 ? 'in' : 'out'}`, options)
        .then(res => {
            if (res.status === 403) return alert('Attendance already added');
